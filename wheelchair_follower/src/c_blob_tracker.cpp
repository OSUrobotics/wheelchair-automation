...
public:
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    cv::grabCut((cv_ptr->image),result,rectangle,bgModel,fgModel,5,cv::GC_INIT_WITH_RECT); // use rectangle
    cv::compare(result,cv::GC_PR_FGD,result,cv::CMP_EQ);
    cv::Mat foreground(cv_ptr->image.size(),CV_8UC3,cv::Scalar(255,255,255));
    cv_ptr->image.copyTo(foreground,result); // bg pixels not copied
    // Update GUI Window
